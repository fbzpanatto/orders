...
export const legalExistsByDoc = async (req: Request, res: Response, next: NextFunction) => {
  ...
  return (await findRegisters(DatabaseTables.legal_persons, 'cnpj', body.cnpj as string) as Array<LegalPerson>).length ?
...
export const normalExistsByDoc = async (req: Request, res: Response, next: NextFunction) => {
  ...
  return (await findRegisters(DatabaseTables.normal_persons, 'cpf', body.cpf as string) as Array<NormalPerson>).length ?
    res.status(409).json(objectResponse(409, 'Dado duplicado.')) : next()
}
